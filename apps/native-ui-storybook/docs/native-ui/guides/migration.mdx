...
There is currently no direct replacement for the `Modal` component from `@utilitywarehouse/uw-native-ui`.
...
+ const styles = useStyles(sttylesheet);
  return (
...
+ import { Box, Text, useStyles, createStyleSheet } from '@utilitywarehouse/native-ui';
...
The old `@utilitywarehouse/uw-native-ui` theme was structured with component-specific properties at the root level:

```tsx
// Legacy
const { spacing, colors, button, input } = theme;
```

The new `@utilitywarehouse/native-ui` theme separates core theme values from component tokens:

```tsx
// New
const { colors, space, fonts, tokens } = theme;
```
...
| -------------------------------- | --------------------------------- | --------------------------------------------------------- |
| `theme.spacing(1)`               | `theme.space[4]`                  | New spacing is in absolute values, 4 is equivalent to 8px |
...
```tsx
import { useLightDark, createStyleSheet, useStyles } from '@utilitywarehouse/native-ui';
