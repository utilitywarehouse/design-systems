...
the `lab` subdirectory, and in this release we have moved `Link` out of `lab`,
replacing the previous `Link` component.

These are the significant API changes you should be aware of.
...
There are codemods to support these changes, but you should still visually check all changes.
...
- The default variant has changed from `primary` to `solid` so you do not need to make any changes if you have not explicitly specified a size.
...
There are codemods to support these changes, but you should still manually
check all changes.
