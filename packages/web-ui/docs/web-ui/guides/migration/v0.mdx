...
This guide should help with migrating from the legacy `customer-ui-material`
component library to **Web UI** v0, and with any `v0` minor version changes to
components in the `lab` subdirectory.

It should cover all significant changes, however there may be some minor
...
- [v0.10](#v010)
  - [lab Button size](#lab-button-size)
...
In `v0.10` we are introducing a new larger size button. The previous `large`
size will be renamed `medium` so we can name the new size `large`.
