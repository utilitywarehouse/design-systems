...
Updates `customer-ui-material` imports. Most components will change to **Web UI** imports, however some legacy components will change to local imports.

Imported usage of `useTheme` will stay as a `customer-ui-material` import. We recommend you update manually, and avoid the use of `useTheme` entirely.
...
Updates usage of the `Background` component to the `Box` component.
