import { Meta, Canvas, ArgTypes, Description } from '@storybook/blocks';
import * as Stories from './Background.stories';

<Meta of={Stories} />

# Background

> This component is deprecated and will be removed in the next major version (`v1`).

<Description />

## Semantic HTML

By default `Background` renders a `div` element, this can be customised using the
`component` prop.

```tsx dark
<Background component="span">...</Box>
```

## Background colours

Additionally to the system properties, Background supports pre-defined background colours
that provide visual context to child components. This means that components
will change foreground colour depending on the parent background colour.

<Canvas
  of={Stories.Backgrounds}
  source={{
    code: `
<Stack spacing={0}>
  <Background backgroundColor="white" padding={2}>
    <Text variant="body" component="span">
      Text on neutral background
    </Text>
  </Background>
  <Background backgroundColor="whiteOwl" padding={2}>
    <Text variant="body" component="span">
      Text on neutral background
    </Text>
  </Background>
  <Background backgroundColor="lightTint" padding={2}>
    <Text variant="body" component="span">
      Text on neutral background
    </Text>
  </Background>
  <Background backgroundColor="purple" padding={2}>
    <Text variant="body" component="span">
      Text on inverse background
    </Text>
  </Background>
  <Background backgroundColor="midnight" padding={2}>
    <Text variant="body" component="span">
      Text on inverse background
    </Text>
  </Background>
</Stack>
    `,
  }}
/>

### Known issues

Foreground `Typography` colours are not correctly applied within nested
`Background` components that are applying different background colours. There
is no plan to fix this. Please use the `Box` component instead.

## Props

<ArgTypes of={Stories} />
