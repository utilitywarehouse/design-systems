...
If the light colour mode is enabled. If you want to enable dark mode, you can pass the `colorMode` prop to the `useColorMode` hook.
The two options for `colorMode` are `light` and `dark`.

```tsx
import { View } from 'react-native';
import { useColorMode, Button } from '@utilitywarehouse/native-ui';
...
const App: React.FC = () => {
  ...
  return (
    <View>
      <Button onPress={() => setColorMode(colorScheme === 'light' ? 'dark' : 'light')}>
        Toggle Dark Mode
      </Button>
    </View>
  );
