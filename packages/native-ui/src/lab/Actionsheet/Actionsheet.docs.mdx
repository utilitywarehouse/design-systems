...
<Meta title="Lab Components / Actionsheet" />
...
const styles = StyleSheet.create(({ space, colors, borderRadius }) => ({
  indicator: {
    width: space['800'],
    height: space['150'],
...
import { Heading, Text, Button, StyleSheet, useTheme } from '@utilitywarehouse/native-ui';
...
const MyComponent = () => {
  ...
  const handleClose = () => setShowActionsheet(!showActionsheet);
  return (
    <Box h={394} alignItems="center" justifyContent="center">
      <Button onPress={handleClose}>
        <ButtonText>Open</ButtonText>
      </Button>
      <Actionsheet
