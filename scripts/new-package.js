...
const copyToNewPackage = async (newPackagePath) => new Promise((resolve, reject) => {
  copy(`${path.resolve(__dirname, "..", "package-template")}${path.sep}*`, newPackagePath, (error) => {
...
const injectPackageVariables = (file, package) => {
  ...
  contents = contents.replace(/{{\s?package\.(name|description)\s?}}/g, (_, key) => package[key]);
...
const main = async () => {
  try {
    ...
    while (!shouldContinue) {
      ...
      shouldContinue = ["y", ""].includes(shouldContinueInput.substring(0, 1).toLowerCase());
    }
    ...
    injectPackageVariables(path.resolve(newPackagePath, "package.json"), package);
    process.exit(0);
  } catch (error) {
    console.error(error);
    process.exit(1);
